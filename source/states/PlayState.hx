...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		*/
		...
		playField = new PlayField(unspawnNotes, dad, boyfriend, noteskins);
		playField.cameras = [camStrum];
		add(playField);

		hudBuild.updateHitbox(downscroll);

		inst = new FlxSound();
		inst.loadEmbedded(Paths.inst(SONG.song, songDiff), false, false);

		vocals = new FlxSound();
		if(SONG.needsVoices)
			vocals.loadEmbedded(Paths.vocals(SONG.song, songDiff, "-player"), false, false);
		...
		if(SONG.needsVoices
		&& Paths.songPath(SONG.song, 'Voices', songDiff, '-opp').endsWith('-opp'))
		{
			vocalsOpp = new FlxSound();
			vocalsOpp.loadEmbedded(Paths.vocals(SONG.song, songDiff, '-opp'), false, false);
			addMusic(vocalsOpp);
		}
	...
	public static var playerSinging:Bool = false;
	...
	{
		var char:Character = playField.dadStrumline.character.char;
		...
		if(sect != null)
		{
			if(forcedCamSection != "none")
				...
				char = playField.bfStrumline.character.char;
		}
	...
	public function startGameOver()
	{
		...
		openSubState(new GameOverSubState(playField.bfStrumline.character));
	}

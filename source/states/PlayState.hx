	...
	function onEventHit(daEvent:EventNote) {
		switch(daEvent.eventName)
		{
			...
			case 'Change HudB Alpha':
				FlxTween.tween(
						hudBuild, {alpha: CoolUtil.stringToFloat(daEvent.value1, 1)},
						CoolUtil.stringToFloat(daEvent.value2, 0),
						{
							ease: CoolUtil.stringToEase(daEvent.value3), // i just like this one i guess :)
						}
				);
